...
export default {
    Query: {
        groups: combineResolvers(isAuthenticated, async (_, __, { dataSources }) => {
            return await dataSources.GroupAPI.getAll();
        }),
        group: combineResolvers(isAuthenticated, async (_, { id }, { dataSources }) => {
    ...
    Group: {
        members: async (group, args, { loaders, dataSources }) => {
            const membersIds = await dataSources.GroupAPI.getAllMembersIdById(group.id);
